...
impl HwDecoder {
    ...
    fn decode(&mut self, data: &[u8], pts: Duration, is_sync: bool) -> Result<()> {
        ...
        if let Some(cfg) = self.pending_config.take() {
            // Prepend SPS/PPS as SEPARATE Annex-B units: some decoders only
            // start emitting once parameter sets arrive in-band even when
            // csd-0 was provided at configure time. `parse_avcc` already
            // emits each unit with its own start code; `avcc_to_annexb_*`
            // does the same for packet data, so no extra separator is needed
            // (a stray zero unit would fuse with the following NALU).
